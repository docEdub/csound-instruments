...
/// @internal
opcode _af_module_{{ModuleName}}_audio_generator, a, kkik
    k_amp, k_cps, i_mode, k_pulseWidth xin

    if (i_mode == {{Source_A.Osc.Wave.Sine}}) then
        a_out = poscil3(k_amp, k_cps)
    else
        i_vco2_mode = \
            i_mode == {{Source_A.Osc.Wave.Saw}} ? {{vco2.Mode.Sawtooth}} : \
            i_mode == {{Source_A.Osc.Wave.Pulse}} ? {{vco2.Mode.Square}} : \
            i_mode == {{Source_A.Osc.Wave.Triangle}} ? {{vco2.Mode.SawtoothTriangleRamp}} : \
            -1
...
opcode AF_Module_{{ModuleName}}, a, S
    ...
    if (k_hostOsc2Enabled == {{true}}) then
        ...
        a_out += _af_module_Source_A_audio_generator(k(1) - k_hostOscMix, cpsmidinn(k_osc2NoteNumber), i_hostOsc2Wave, k_hostOsc2PulseWidth)
    endif

    if (k_hostSubEnabled == {{true}}) then
        if (i_hostSubWave == {{Source_A.Sub.Wave.Pulse}}) then
            i_vco2_mode = {{vco2.Mode.SquareWaveNoPWM}}
        elseif (i_hostSubWave == {{Source_A.Sub.Wave.Triangle}}) then
