...
///
opcode {{Module_public}}, a, SkO
    S_channelPrefix, k_noteNumber, k_cpsOffset xin
    ...
    if (k_hostOsc1Enabled == $true) then
        ...
        a_out += {{Module_private}}_audio_generator(k_hostOscMix, cpsmidinn(k_osc1NoteNumber) + k_cpsOffset, i_hostOsc1Wave, k_hostOsc1PulseWidth)
    endif

    if (k_hostOsc2Enabled == $true) then
        k_osc2NoteNumber = k_noteNumber + k_hostOsc2Semi + k_hostOsc2Fine / 100
        a_out += {{Module_private}}_audio_generator(k(1) - k_hostOscMix, cpsmidinn(k_osc2NoteNumber) + k_cpsOffset, i_hostOsc2Wave, k_hostOsc2PulseWidth)
    endif
